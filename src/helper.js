...
const resetMode = (
  ...
  setLanguageResources((prev) => {
    ...
    defaultTpl[currentLanguage].lg.forEach((el) => {
      if (!new_val.includes(el.i.split('/')[1].split('_')[0])) {
        new_val.push(el.i.split('/')[1].split('_')[0]);
      }
    });
    return new_val;
  });
...
export const getLayoutType = (layout) => {
  let type = 'bible';
  layout.forEach((el) => {
    if (el.i.split('/')[1].split('_')[1].split('-')[0] === 'obs') {
      type = 'obs';
    }
  });
  return type;
...
export const getLanguageIds = () => {
  ...
  if (allValues) {
    allValues.forEach((value) => {
      value.lg.forEach((el) => {
        currentLangs.add(el.i.split('/')[1].split('_')[0]);
      });
    });
  }
